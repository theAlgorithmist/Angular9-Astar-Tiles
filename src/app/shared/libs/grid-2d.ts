...
import { IGridView } from '../interfaces/grid-view';

import { CELL_TYPE } from '../interfaces/cell-type';
...
export class Grid2D
{
  ...
  */
  public get targetNode(): TileNode
  {
  ...
  public isReachable(i: number, j: number, reachable: boolean): void
  {
    if (i >= 0 && i <= this._numRows)
    {
      if (j >= 0 && j <= this._numCols)
      {
        ...
        if (this._gridView != null && !reachable) {
          this._gridView.setCell(i, j, CELL_TYPE.BARRIER);
        }
      }
    }
  }
  ...
  */
  public isOccupied(i: number, j: number, occupied: boolean): void
  {
    if (i >= 0 && i <= this._numRows)
    {
      if (j >= 0 && j <= this._numCols)
      {
        let indicator: CELL_TYPE = occupied ? CELL_TYPE.OCCUPIED : CELL_TYPE.OPEN;

        if (this._gridView != null) {
          this._gridView.setCell(i, j, indicator);
        }
      }
    }
  }
  ...
  public isHazard(i: number, j: number, cost: number): void
  {
    if (i >= 0 && i <= this._numRows)
    {
      if (j >= 0 && j <= this._numCols)
      {
        ...
        if (this._gridView != null) {
          this._gridView.setCell(i, j, CELL_TYPE.HIGH_COST);
        }
      }
    }
  }
  ...
  public setStartNode(i: number, j: number): void
  {
    if (i >= 0 && i <= this._numRows)
    {
      if (j >= 0 && j <= this._numCols)
      {
        ...
        if( this._gridView ) {
          this._gridView.setCell(i, j, CELL_TYPE.START);
        }
      }
    }
  }
  ...
  public setTargetNode(i: number, j: number): void
  {
    if (i >= 0 && i <= this._numRows)
    {
      if (j >= 0 && j <= this._numCols)
      {
        ...
        if (this._gridView) {
          this._gridView.setCell(i, j, CELL_TYPE.END);
        }
      }
    }
  }
